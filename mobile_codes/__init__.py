...
MNC_OPERATOR_JSON_PATH = os.path.join(FILE_PATH, 'json', 'operators.json')
COUNTRY_FIELDS = ('name', 'alpha2', 'alpha3', 'numeric', 'mcc')
MNC_OPERATOR_FIELDS = ('mcc', 'mnc', 'brand', 'operator')
SID_OPERATOR_FIELDS =  [
    "sid", "high_sid", "quantity", "assignee", "country", "state",
    "expiry_date", "conflict", "conflicting_country", "frequency",
    "technology"]

Country = namedtuple('Country', COUNTRY_FIELDS)
MNCOperator = namedtuple('Operator', MNC_OPERATOR_FIELDS)
...
def _mnc_operators():
    return _load_json('mnc_operators_json', MNC_OPERATOR_JSON_PATH, MNCOperator)
...
operators = partial(_get, _mnc_operators, 'mnc_operators', _build_list_index, 0)
